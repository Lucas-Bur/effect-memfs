...
it.effect("layerWith populates filesystem", () =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem
    const content = yield* fs.readFileString("/hello.txt")
    expect(content).toBe("World")
  }).pipe(Effect.provide(layerWith({ "/hello.txt": "World" }))),
)

it.effect("layerWith creates independent state per provide", () =>
  Effect.gen(function* () {
    const program1 = Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      yield* fs.writeFileString("/x.txt", "modified")
      return yield* fs.readFileString("/x.txt")
    }).pipe(Effect.provide(layerWith({ "/x.txt": "1" })))
